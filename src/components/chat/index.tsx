...
const ChatBox = () => {
  ...
  return (
    <>
      {isShow && (
        <div className="fixed bottom-28 right-4 w-[90vw] max-w-[400px] 0 bg-blue-100 z-100 rounded-md shadow-lg rounded-bl-none rounded-br-none">
          <div className="flex flex-col max-h-80 overflow-y-auto  p-4 mb-4 gap-3 chat-scroll" ref={scrollRef}>
            {groupedMessages.map((group, i) => {
              const isMine = user.nickname === group.nickname;
              return (
                <div key={i} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`flex flex-col ${isMine ? 'items-end text-right' : 'items-start text-left'} max-w-[75%]`}
                  >
                    {!isMine && <span className="text-sm mb-1">{group.nickname}</span>}
                    <div className="flex flex-col space-y-1">
                      {group.items.map((msg, j) => (
                        <div key={msg.id}>
                          <p
                            className={`inline-block tossface px-3 py-2 rounded-md text-sm break-words ${
                              isMine
                          ...
                          {j === group.items.length - 1 && (
                            <span className="text-[12px] text-gray-700 mt-1 block">
          ...
          <div className="flex gap-2 items-center border-t border-t-gray-300 bg-white  px-4 py-2">
            <input
              className="flex-1 border border-gray-300 rounded-md p-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 placeholder:text-gray-700"
              value={input}
            ...
            />
            <Button change={handleSend} className="w-[84px]" option={{ isIcon: true }} text={''} aria-label="전송" />
          </div>
        </div>
      )}
      <button
